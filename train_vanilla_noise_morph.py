...
class ReplayBuffer:
    ...
    def push(self, o, a, r, o_1): # o and o_1 will be potentially noisy observations
...
class Q_FC(torch.nn.Module):
    def __init__(self, obs_size, action_size): # obs_size will be base_obs_size
...
class SAC:
    def __init__(self, arglist):
        ...
        if torch.cuda.is_available() and self.arglist.use_gpu:
        ...
        try:
            # Get obs_size directly from the environment (no manual augmentation)
            with HalfCheetahMorphEnv(xml_path=self.xml_path, use_custom_morphology=False, render_mode=False) as temp_env:
                self.obs_size = temp_env.observation_space.shape[0] # This is the base_obs_size
                self.action_size = temp_env.action_space.shape[0]
        except Exception as e:
            print(f"Error creating temporary environment to determine obs_size: {e}")
            raise
        ...
        # Networks are initialized with the direct obs_size from env
    ...
    # Removed _augment_observation_if_needed as it's not used when no leg length input
    ...
    def _load_checkpoint(self):
        ...
        try:
            ...
            # IMPORTANT: Ensure obs_size of loaded model matches current self.obs_size
            # This version of the script assumes no manual augmentation, so self.obs_size is base_obs_size.
            # If resuming a model trained with manual augmentation, this will cause a size mismatch.
            ...
            if 'replay_buffer_content' in checkpoint and checkpoint['replay_buffer_content'] is not None:
                for item in checkpoint['replay_buffer_content']:
                    # Check if obs in buffer matches current expected obs_size
                    if item[0].shape[0] != self.obs_size:
                        print(f"WARNING: Observation in loaded replay buffer (size {item[0].shape[0]}) "
                              f"does not match current agent obs_size ({self.obs_size}). "
                              "This might happen if resuming a model trained with different obs specs. "
                              "Skipping this item from replay buffer.")
                        continue
            ...
            import traceback
            traceback.print_exc() # Print more details on error
            self.start_episode = 0
            self.total_steps = 0
            self.log_alpha = torch.tensor(np.log(self.arglist.initial_alpha), dtype=torch.float64, device=self.device, requires_grad=True)
            self._initialize_optimizers()
    ...
    def log_video_to_wandb(self, episode_num, leg_lengths_dict, floor_friction_list):
        ...
        local_video_dir = os.path.join(self.exp_dir, "local_videos_no_leg_input_noise") # Updated suffix
        ...
        try:
            ...
            obs_for_video_actor = video_env.reset().astype(np.float64) # Direct from env, ensure dtype
            ...
            for _step in range(self.max_episode_steps):
                ...
                next_obs_for_video_actor, _, env_done, _ = video_env.step(action.astype(np.float32))
                obs_for_video_actor = next_obs_for_video_actor.astype(np.float64)
    ...
    def train(self):
        ...
        # Log training noise std
        wandb.config.update({"train_obs_noise_std": self.arglist.train_obs_noise_std}, allow_val_change=True)

        print(f"Starting training for {self.arglist.episodes} episodes with obs noise std: {self.arglist.train_obs_noise_std}.")
        print(f"Agent is NOT using manually augmented leg length observations.")
        ...
        # Leg lengths are still defined for the environment, but not passed to the agent
        current_leg_lengths = {
        ...
        for episode_idx in range(self.start_episode, self.arglist.episodes):
            ...
            # Observation directly from env, then add noise
            obs_for_agent = training_env.reset().astype(np.float64)
            ...
            for t in range(self.max_episode_steps):
                ...
                # Observation directly from env, then add noise for next state
                next_obs_for_agent = raw_next_obs_from_env.astype(np.float64)
...
def parse_args():
    parser = argparse.ArgumentParser("SAC Training: NO LEG LENGTH INPUT, with Observation Noise, Custom HalfCheetah, Random Morphology, WandB")
    parser.add_argument("--mode", type=str, default="train", choices=["train", "eval"], help="Execution mode")
    parser.add_argument("--seed", type=int, default=0, help="Random seed")
    parser.add_argument("--use_gpu", action=argparse.BooleanOptionalAction, default=True, help="Use GPU if available. Default: True. Use --no-use-gpu to disable.")
    
    # --- MODIFIED: log_dir for this specific experiment ---
    parser.add_argument("--log_dir", type=str, default="./logs_sac_morph_noleginput_obsnoise", help="Base directory for logs and models")

    # --- REMOVED: --manual_obs_augmentation as it's implicitly False for this script version ---

    # --- Argument for training observation noise (still relevant) ---
    ...
    parser.add_argument("--morph_change_every", type=int, default=20, help="Episodes to change morphology")
    ...
    # --- MODIFIED: W&B Naming for this specific experiment ---
    parser.add_argument("--wandb_project_name", type=str, default="SAC_HC_NoLegInput_ObsNoise", help="WandB project name")
    parser.add_argument("--wandb_run_name_suffix", type=str, default="noleginput_obsnoise", help="Suffix for WandB run name")

    # --- Video logging off by default ---
    ...
    parser.add_argument("--video_log_every", type=int, default=250, help="Frequency (episodes) to log videos IF enabled")
...
if __name__ == '__main__':
    args = parse_args()
    # Ensure --manual_obs_augmentation is effectively False for this script's logic
    # This can be done by not having the arg, or by setting it explicitly if the SAC class still checks for it.
    # For this version, we've removed the check from SAC init logic related to manual_obs_augmentation.
    # The SAC class will now always use the base_obs_size.
    
    # If you want to be absolutely explicit you could add this, but it's not strictly needed
    # if the SAC class no longer branches on it for obs_size calculation:
    # args.manual_obs_augmentation = False 
