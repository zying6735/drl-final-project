...
import wandb
import imageio # <--- ADDED IMPORT FOR LOCAL VIDEO SAVING

# Import your custom environment
from halfcheetah_morph_env import HalfCheetahMorphEnv # Assumes this file is in the same directory
...
class Q_FC(torch.nn.Module):
    ...
    def forward(self, x, a):
        if x.ndim > 1 and a.ndim == 1 and a.shape[0] == self.action_size :
             a = a.unsqueeze(0).repeat(x.shape[0], 1) # Repeat action for batch
        elif x.ndim == 1 and a.ndim > 1 and a.shape[0] == 1: # Single obs, batched action (unlikely here)
            a = a.squeeze(0)
        elif x.ndim > 1 and a.ndim > 1 and x.shape[0] != a.shape[0]:
            if a.shape[0] == 1 and x.shape[0] > 1: # Action is (1, action_dim), obs is (batch, obs_dim)
...
class SAC:
    def __init__(self, arglist):
        ...
        try:
            # Use a context manager for the temporary environment
    ...
    # <--- MODIFIED log_video_to_wandb FUNCTION --- START --->
    def log_video_to_wandb(self, episode_num, leg_lengths_dict, floor_friction_list):
        ...
        try:
            ...
            for _step in range(self.max_episode_steps): 
                with torch.no_grad():
                    o_tensor = torch.tensor(obs, dtype=torch.float64, device=self.device).unsqueeze(0)
                    a_tensor, _ = self.actor(o_tensor, deterministic=True)
                action = a_tensor.cpu().numpy().squeeze() # Squeeze action
                
                obs, _reward, env_done, _info = video_env.step(action.astype(np.float32))
                frame = video_env.render(mode='rgb_array')
                if frame is not None:
                    if _step < 5 or _step % 100 == 0 : # Print info for first few and some later frames
                ...
                else: # Should not happen if render_mode=True and render() is called
            ...
            if collected_frames: # Check if list is not empty
                ...
                if video_np.ndim != 4 or video_np.shape[0] == 0 or video_np.shape[3] != 3:
                    ...
                    # Ensure correct dtype (uint8) and range (0-255) for video
                    if video_np.dtype != np.uint8:
                        ...
                        else: # Assume it's already 0-255 range, just wrong type, or clip
            ...
            if video_env:
                video_env.close()
    # <--- MODIFIED log_video_to_wandb FUNCTION --- END --->
    ...
    def train(self):
        ...
        friction_val = random.uniform(0.05, 0.6)
        ...
        for episode_idx in range(self.start_episode, self.arglist.episodes):
            ...
            try:
                training_env = HalfCheetahMorphEnv(
                    xml_path=self.xml_path,
                    leg_lengths=current_leg_lengths,
            ...
            for t in range(self.max_episode_steps):
                if self.total_steps < self.arglist.start_steps:
                    ...
                    with torch.no_grad():
                        obs_tensor = torch.tensor(obs, dtype=torch.float64, device=self.device).unsqueeze(0)
                        action_tensor, _ = self.actor(obs_tensor, deterministic=False)
                    action = action_tensor.cpu().numpy().squeeze()
                
                next_obs, reward, env_terminated, info = training_env.step(action.astype(np.float32))
        ...
        if self.arglist.log_video or self.arglist.log_video_local: # Check if any video logging is enabled
...
def parse_args():
    parser = argparse.ArgumentParser("SAC with Custom HalfCheetah, Random Morphology, and WandB Logging")
    ...
    parser.add_argument("--log_dir", type=str, default="./logs_sac_morph", help="Base directory for logs and models")
    ...
    parser.add_argument("--morph_change_every", type=int, default=20, help="Frequency (episodes) to change morphology")
    ...
    parser.add_argument("--wandb_project_name", type=str, default="SAC_HalfCheetahMorph_exac", help="WandB project name")
    parser.add_argument("--wandb_run_name_suffix", type=str, default="default_run_exac", help="Suffix for WandB run name")
    
    # <--- MODIFIED/ADDED VIDEO LOGGING ARGS --- START --->
    parser.add_argument("--log_video", action="store_true", default=False, help="Log videos of agent rollouts to WandB")
    parser.add_argument("--log_video_local", action="store_true", default=False, help="Log videos locally to a file in the experiment directory") # New arg
    parser.add_argument("--video_log_every", type=int, default=10, help="Frequency (in episodes) to log videos") # Reduced for faster testing
    parser.add_argument("--video_fps", type=int, default=30, help="FPS for logged videos")
    # <--- MODIFIED/ADDED VIDEO LOGGING ARGS --- END --->
